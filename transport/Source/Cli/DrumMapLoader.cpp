...
std::string loadDrumMapFromFile (const std::string& path, DrumMap& target)
{
    ...
    for (const auto& entry : obj->getProperties())
    {
        ...
        // Keys starting with "_" are treated as inline documentation and
        // skipped. This lets users embed comments (e.g. "_help",
        // "_source", "_version") in the JSON without our loader complaining.
        if (! keyText.empty() && keyText.front() == '_')
            continue;
