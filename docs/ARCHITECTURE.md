...
The translation layer between `SongDocument`'s `ValueTree` and `forge_core`'s plain `Config`/`Song` structs — the only place in `Source/UI/` that touches both. Two free functions in namespace `lotro`:

- **`appendImportedSong(SongDocument&, const Song& imported, int importBatch)`** — the import path. Appends `imported`'s tracks (via non-undoable `addTrackBulk`/`appendChildBulk`) and its `tempoMap`/`meterMap` (as `TEMPO_CHANGE`/`METER_CHANGE` children) as new document state, minting fresh `trackId`s and copying `Note` fields verbatim. Accumulates across multiple imports rather than clearing — a second import's tracks/tempo/meter entries are appended alongside the first's. The document's `ticksPerQuarter` is set only by the *first* import into an empty document; a later import's differing value is silently left unapplied (rescaling a second file's ticks into an already-established time base is an open decision, not yet owned by any phase).
